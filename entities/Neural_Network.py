...
class NeuralNetwork:
    def __init__(self, inputs, hidden, outputs, learning_rate=0.1):
        ...
        ## lets trick numpy version incompatibility by changing np.float128 to np.float64
        
        # create weights and gradients for first hidden layer (defined based on number of inputs)
        self.weights = [np.random.uniform(-1, 1, size=(self.hidden[0], self.inputs)).astype(np.float64)]
        self.gradients = [np.zeros((self.hidden[0], self.inputs), np.float64)]
        ...
        if self.hidden.ndim > 0:
            for idx, hidden_col in enumerate(self.hidden[1:]):
                self.weights.append(np.random.uniform(-1, 1, size=(hidden_col, self.hidden[idx])).astype(np.float64))
                self.gradients.append(np.zeros((hidden_col, self.hidden[idx]), np.float64))

        # create weights and gradients for output layer (defined based on number of outputs)
        self.weights.append(np.random.uniform(-1, 1, size=(self.outputs, self.hidden[-1],)).astype(np.float64))
        self.gradients.append(np.zeros((self.outputs, self.hidden[-1],), np.float64))
        ...
        for idx, hidden_col in enumerate(self.hidden):
            self.bias.append(np.random.uniform(-1, 1, size=(hidden_col, 1)).astype(np.float64))
            self.bias_gradients.append(np.zeros((hidden_col, 1), np.float64))
        self.bias.append(np.random.uniform(-1, 1, size=(self.outputs, 1)).astype(np.float64))
        self.bias_gradients.append(np.zeros((self.outputs, 1), np.float64))
    ...
    def gradient_zeros(self):
        self.gradients = [np.zeros((self.hidden[0], self.inputs), np.float64)]
        self.bias_gradients = [np.zeros((self.hidden[0], 1), np.float64)]
        if self.hidden.ndim > 0:
            for idx, hidden_col in enumerate(self.hidden[1:]):
                self.gradients.append(np.zeros((hidden_col, self.hidden[idx]), np.float64))
                self.bias_gradients.append(np.zeros((hidden_col, 1), np.float64))
    ...
    def save_to_file(self, file_name=None):
        if not file_name:
            file_name = f'NeuralNet_{c.N}.json'
        json_file = {
