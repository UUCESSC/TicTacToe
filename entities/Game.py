...
class Game:
    def __init__(self):
        self.state = np.zeros((c.N, c.N), np.int8)

        ## if the size is odd, make the middle cell unchoosable
        if c.N % 2 == 1:
            self.state[int((c.N-1)/2), int((c.N-1)/2)] = -1
        self.PLAY_SPRITES = []
        self.score = [0, 0]
        self.winner = 0
        self.turn = 1
            ...
            for row_col in range(c.N):
                for col_row in range(c.N):
                    ...
                    if self.state[row_col, col_row] == 0 and \
                            np.all(self.state[row_col, (np.arange(c.N) != col_row)] == player):
                        return row_col, col_row
                    # check cols
                    if self.state[col_row, row_col] == 0 and \
                            np.all(self.state[(np.arange(c.N) != col_row), row_col] == player):
                ...
                if self.state[row_col, row_col] == 0 and \
                        np.all(diagonal[(np.arange(c.N) != row_col)] == player):
                    return row_col, row_col
                # check secondary diagonal
                diagonal = np.diagonal(np.flipud(self.state))
                if self.state[c.N - 1 - row_col, row_col] == 0 and \
                        np.all(diagonal[(np.arange(c.N) != row_col)] == player):
                    return c.N - 1 - row_col, row_col

        ## checks all 6 cells one by one in size=3. not applicable for n>3 
        """# else check for corner threats
        for i in range(2):
            if self.state[0, i * (c.N-1)] == adversary and \
                    self.state[(c.N-1), (c.N-1) - (i * (c.N-1))] == adversary and \
                    np.all(self.state[0, (1 - i, (c.N-1) - i)] == 0) and \
                    np.all(self.state[(i, 1 + i), (c.N-1)] == 0) and \
                    np.all(self.state[(1 - i, (c.N-1) - i), 0] == 0) and \
                    np.all(self.state[(c.N-1), (i, 1 + i)]) == 0:
                return 0, 1"""
            ...
            for row_col in range(c.N):
                for col_row in range(c.N):
                    ...
                    if self.state[row_col, col_row] == player and \
                            np.all(self.state[row_col, (np.arange(c.N) != col_row)] == 0):
                        p = [p for p in range(c.N) if p != col_row]
                        for i in p:
                            q = [q for q in range(c.N) if q != row_col]
                            for j in q:

                                # dont choose nor examine the middle cell
                                # if row_col == (c.N-1)/2 and i == (c.N-1)/2:
                                #     continue
                                
                                if self.state[j, i] == player and \
                                        np.all(self.state[(np.arange(c.N) != j), i] == 0):
                                    return row_col, i

        ## Do not play the middle cell
        """# else play center
        if self.state[1, 1] == 0:
            return 1, 1"""

        # else play corner
        for y in range(0, c.N, c.N-1):
            for x in range(0, c.N, c.N-1):
                if self.state[y, x] == 0:
                    return y, x
    ...
    def random_play(self):
        ...
        # ## if the middle cell is chosen randomly, repeat the random choice process
        # if x == (c.N-1)/2 and y == (c.N-1)/2:
        #     self.random_play()
        return y, x
